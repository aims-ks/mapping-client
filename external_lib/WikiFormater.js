...
export default class WikiFormatter {
	...
	format(input) {
		...
		let output = '', currentChar = '', htmlChunk = '';
		...
		for (let i=0, len=input.length; i<len; i++) {
			...
			// * Styles (<b>, <i>, <u>, <s>) *
			// *******************************
			if (WikiFormatter._isStyleChar(currentChar)) {
				const openTag = WikiFormatter._getStyleTag(input, i);
				if (openTag && openTag.open) {
					// Look for its close tag
					const closeTag = this._findCloseTag(input, openTag);
					if (closeTag) {
			...
			if (currentChar === '[' && i+1 < len && input.charAt(i+1) === '[') {
				let index = i, inURL = true, isURL = false;
				...
				if (isURL) {
					let wikiURL = input.substring(i+2, index-1);
					let urlStr, filename, label, dim, cssClass, wikiURLParts = wikiURL.split('|');
					...
					if (type === 'IMG') {
						...
						if (dim) {
							dim = dim.split('X');
							let width = dim[0];
							let height = dim.length > 1 ? dim[1] : null;
							if (width && width !== '?') {
			...
			// * Complete URLs (http://...) *
			// ******************************
			if (WikiFormatter._isCompleteURL(input, i)) {
				let urlStr = WikiFormatter._toURL(input, i);
				if (urlStr) {
					let label = this._truncateURLForDisplay(urlStr);
			...
			// * Incomplete URLs (www...) *
			// ****************************
			if (WikiFormatter._isIncompleteURL(input, i)) {
				let urlStr = WikiFormatter._toURL(input, i);
				if (urlStr) {
					let label = this._truncateURLForDisplay(urlStr);
			...
			if (currentChar === '=' && (i <= 0 || input.charAt(i-1) === '\n')) {
				...
				let headerTagNumber = Math.min(openTag.length, closeTag.length) - 1;
			...
			// * Bullet list *
			// ***************
			if (WikiFormatter._isListLine(input, i)) {
				// Collect all lines that define the list
				let index = i, inList = true;
				while (inList && index < len) {
					if (index > 0 && input.charAt(index-1) === '\n') {
						...
						// of the bullet list.
						inList = WikiFormatter._isListLine(input, index);
					}
					if (inList) {
						index++;
					}
				}
				let listBlock = input.substring(i, index);

				htmlChunk += this._createHTMLList(WikiFormatter._createListObjFromWikiFormat(listBlock));
				i = index-1;
				currentChar = i < len ? input.charAt(i) : '';
			}
			...
			// * Numbered list *
			// *****************
			if (WikiFormatter._isListLine(input, i, true)) {
				// Collect all lines that define the list
				let index = i, inList = true;
				while (inList && index < len) {
					if (index > 0 && input.charAt(index-1) === '\n') {
						...
						// It's time to check if the line is still part
						// of the bullet list.
						inList = WikiFormatter._isListLine(input, index, true);
					}
					if (inList) {
						index++;
					}
				...
				htmlChunk += this._createHTMLList(WikiFormatter._createListObjFromWikiFormat(listBlock, true), true);
				i = index-1;
				currentChar = i < len ? input.charAt(i) : '';
			}
  ...
  static _isStyleChar(char) {
	 ...
	 * @private
	 */
  static _isListLine(input, index, numbered) {
		let len = input.length, bulletChar = numbered ? '#' : '*';
	 ...
	 * @private
	 */
  static _getStyleTag(input, index) {
		let tag = {
		...
		let styleDelimiterInRegex = /[\w:\.,\[\]\(\){}]/;
		let styleDelimiterOutRegex = /[^\w:]/;

		// Check if the sequence start with white space
		let len = input.length, startIndex = index, endIndex = index;
		while (startIndex-1 >= 0 && WikiFormatter._isStyleChar(input.charAt(startIndex-1))) {
			startIndex--;
		}
		while (endIndex+1 < len && WikiFormatter._isStyleChar(input.charAt(endIndex+1))) {
			endIndex++;
		}
	 ...
	 * @private
	 */
	_findCloseTag(input, openTag) {
		let tag, closeTag, currentChar = '';
		for (let i=openTag.index+1, len=input.length; i<len; i++) {
			currentChar = input.charAt(i);
			if (WikiFormatter._isStyleChar(currentChar)) {
				tag = WikiFormatter._getStyleTag(input, i);
	 ...
	 * @private
	 */
  static _createListObjFromWikiFormat(listStr, numbered) {
		let bulletChar = numbered ? '#' : '*';
		let valueRegex = numbered ? /^#+\s+/ : /^\*+\s+/;

		// Split on '\r' (Mac), '\n' (UNIX) or '\r\n' (Windows)
		let bulletListItems = listStr.replace(/[\n\r]+/g, '\n').split('\n');

		let bulletListObj = [];
		for (let i=0, len=bulletListItems.length; i<len; i++) {
			let itemStr = bulletListItems[i];

			let value = itemStr.replace(valueRegex, '');
			let index = 0;
			let listPtr = bulletListObj;
	 ...
	 * @private
	 */
	_createHTMLList(list, numbered) {
		let listTagName = numbered ? 'ol' : 'ul';

		let htmlList = '<'+listTagName+'>\n';
	 ...
	 * @private
	 */
	_truncateURLForDisplay(url) {
		let maxUrlLength = this.MAX_URL_LENGTH || 40;
		...
		if (maxUrlLength > 0 && maxUrlLength < url.length) {
			let beginningLength = Math.round((maxUrlLength-3) * 3.0/4);
			let endingLength = maxUrlLength - beginningLength - 3; // 3 is for the "..."
	 ...
	 * @private
	 */
  static _isCompleteURL(input, i) {
		let inputChunk = input.substr(i, 10);
	 ...
	 * @private
	 */
  static _isIncompleteURL(input, i) {
		let inputChunk = input.substr(i, 5);
	 ...
	 * @private
	 */
  static _toURL(input, i) {
		let urlCharRegex = /[a-zA-Z0-9\$\-_\.\+\!\*'\(\),;\/\?:@=&#%]/,
