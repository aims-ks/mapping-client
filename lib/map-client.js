...
import MapClickEventEmitter from './events/map-click-event-emitter';
  ...
  init() {
    ...
    return ConfigLoader(this._options.mapConfigHost, this._options.mapConfigURL)
      .then((mapConfig) => {
        ...
        // create map click event emitter
        const mapClickEventEmitter = new MapClickEventEmitter(this._olMap);
        mapClickEventEmitter.on(mapClickEventEmitter.EVENT_SINGLE_CLICK, function(event) {
          console.log('MapClickEventEmitter.EVENT_SINGLE_CLICK triggered: ', event);
        });
