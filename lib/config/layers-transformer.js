...
module.exports = {
  ...
  _transformNode(_node, _id, _parentId) {
    ...
    if (_node.title) {
      ...
      const layer = this._makeLayer(_node, _id, _parentId);
      list.push(layer);

    } else {

      // Not a Layer, so treat as a parent node. Add this to the list if an `_id` is defined.
      // Iteratively invoke this method for every property of the node.
      if (_id) {
        const parent = this._makeParent(_node, _id, _parentId);
        list.push(parent);
      }
      Object.keys(_node).forEach(function (_key) {
        list = list.concat(this._transformNode(_node[_key], _key, _id));
      }.bind(this));
    }
  ...
  _processLayer(layerList, dataSources) {
    ...
    layerList.forEach((_layer) => {
      ...
      _layer['hasLegend'] = (_layer['hasLegend'] === 'false' ? false : true);
      ...
      if (dataSource) {
        if (!_layer['wmsFeatureRequestLayers']) {
          ...
          var requestLayers = _layer['wmsFeatureRequestLayers'];
          for (var i = 0; i < requestLayers.length; i++) {
            if (typeof(requestLayers[i]) === 'string') {
              var _layerId = requestLayers[i];
              requestLayers[i] = {};
      ...
      if (dataSource) {
        for (var dataSourceProp in dataSource) {
