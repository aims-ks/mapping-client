...
export default class AddLayersTab {
   ...
   * Constructor.
   */
  constructor(availableLayersModel, supportedLayersModel, mapConfig, supportedLayersManager) {
  ...
  save() {
    ...
    $selections.each((index, element) => {
      ...
      // Add the layer to the Available layers if it hasn't already been added.
      if (_supportedLayer) {
        const isFound = _availableLayers.some((_availableLayer) => {
