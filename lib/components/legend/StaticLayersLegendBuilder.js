import AbstractLegendBuilder from './AbstractLegendBuilder';
...
export default class StaticLayersLegendBuilder extends AbstractLegendBuilder {
  ...
  buildHtml() {
    ...
    this._activeLayersModel.getLayers().reverse().forEach((layer) => {
      ...
      const dataSource = this._dataSources[layer.dataSourceId] || layer;

      // Are legends displayed for this datasource?
      if (
        dataSource
        && dataSource.showInLegend
        && dataSource.legendUrl
        && layer.hasLegend === true
      ) {

        // Build the legend url.
        let url = `${dataSource.legendUrl}TRANSPARENT=TRUE&REQUEST=GetLegendGraphic&LAYER=${layer.shortLayerId}&SCALE=3466743.325731736`;
        ...
        html += `
          <div class="legend-item">
            <div class="legend-title">${layer.title}</div>
