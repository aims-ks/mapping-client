...
export default class ComponentsBuilder {
  ...
  build(options, $target) {
    ...
    if (!options.disableShowLayers) {
      ...
      this._buildShowLayersComponent(this._isDefaultActiveComponent(options, 'showLayers'));
    }

    if (!options.disableLegend) {
      this._buildLegendComponent(this._isDefaultActiveComponent(options, 'legend'));
    }

    if (!options.disableFeatureRequests) {
      this._buildGetFeatureInfoComponent(options, this._isDefaultActiveComponent(options, 'getFeatureInfo'));
    }
   ...
   * @private
   */
  _isDefaultActiveComponent(options, componentId) {
  ...
  _buildGetFeatureInfoComponent(options, activate) {
    ...
    switch (options.featureRequestsResultsPosition) {
      ...
      default:
        ...
        getFeaturesControlButton = this._userInterfaceComponent.getRightControlsPanel().createButton(
          'aims-map-get-features',
          'Query features',
          'pageview',
          true
    ...
    resultPanel.on(resultPanel.EVENT_CLOSED, (event) => {
