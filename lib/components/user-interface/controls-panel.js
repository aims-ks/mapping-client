...
export default class ControlsPanel extends EventEmitter {
   ...
   */
  createButton(className, title, icon, toggleState) {
    ...
    button.on(button.EVENT_DEACTIVATED, (deactivatedButton) => {
      if (this._buttons.filter(button => button.isActive()).length === 0) {
