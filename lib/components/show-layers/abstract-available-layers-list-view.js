...
export default class AbstractAvailableLayersListView extends EventEmitter {
  ...
  handleChange() {
    ...
    this._availableLayersModel.getLayers().reverse().forEach(function (_availableLayer) {
      let status = {};
      // if status has been saved previously, restore it
      if (prevLayerStatus[_availableLayer.layerId]) {
        status = prevLayerStatus[_availableLayer.layerId];
      }

      this._internalCache.push(_availableLayer);
      status.selected = this._activeLayersModel.getLayers().some((_activeLayer) => {
        return _activeLayer.layerId === _availableLayer.layerId;
      });
      this._$el.append(this.renderLayer(_availableLayer, status));
    }, this);
