import StaticLayersLegendBuilder from './legend/StaticLayersLegendBuilder';
import AddLayersDialog from './add-layers/AddLayersDialog';
import ShowLayersDialog from './show-layers/ShowLayersDialog';
import Legend from './legend/LegendComponent';
import UserInterfaceComponent from './user-interface/UserInterfaceComponent';
import ActionButton from './user-interface/ActionButton';
import GetFeatureInfoComponent from './get-feature-info/GetFeatureInfoComponent';
...
export default class ComponentsBuilder {
  ...
  build(options, $target) {
    ...
    if (!options.disableShowLayers) {
      ...
      this._buildShowLayersComponent(ComponentsBuilder.isDefaultActiveComponent(options, 'showLayers'));
    }

    if (!options.disableLegend) {
      this._buildLegendComponent(ComponentsBuilder.isDefaultActiveComponent(options, 'legend'));
    }

    if (!options.disableFeatureRequests) {
      this._buildGetFeatureInfoComponent(options, ComponentsBuilder.isDefaultActiveComponent(options, 'getFeatureInfo'));
    }

    if (!options.disableMetadata) {
      this._buildMetadataComponent(ComponentsBuilder.isDefaultActiveComponent(options, 'metadata'));
    }
  ...
  /**
   * Get the AddLayersDialog component
  ...
  /**
   * Get the ShowLayersDialog component
   ...
   * @private
   */
  static isDefaultActiveComponent(options, componentId) {
