...
import GetFeatureInfoElement from './GetFeatureInfoElement';
...
export default class GetFeatureInfoComponent {
  ...
  constructor(olMap, layerModel, params, mapConfig) {
    ...
    this._$featuresEl = $('<div class="mapping-client-get-features"><p>Click on the map to request a feature</p></div>');
  ...
  request(event) {
    ...
    const forceHttps = window.location.protocol === 'https:';
    ...
    const olLayers = this._olMap.getLayers().getArray();
    olLayers.forEach((olLayer) => {
      const source = olLayer ? olLayer.getSource() : null;

      if (source && typeof source.getFeatureInfoUrl === 'function') {
        const layer = this._layerModel.findByLayerId(olLayer.get('layerId'));

        if (layer && layer.wmsQueryable === true) {
          let featureInfoElement = this._elementList
            .find((element) => element.layerId === layer.layerId);
          ...
          let url = source.getFeatureInfoUrl(
            ...
            viewResolution,
            projection,
            { QUERY_LAYERS: layer.shortLayerId, ...this._params },
          );
    ...
    } else if (this._$featuresEl.find('.mapping-client-collapsible-element-content-open').length === 0 && firstFeatureInfoElement.keepState === false) {
