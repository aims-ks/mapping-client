...
export default class GetFeatureInfoComponent {
   ...
   * @param params
   */
  constructor(olMap, layerModel, params) {
    this._olMap = olMap;
  ...
  request(event) {
    ...
    this._olMap.forEachLayerAtPixel(pixel, olLayer => {
      ...
      if (layer && layer.wmsQueryable === true) {
        ...
        let url = olLayer.getSource().getGetFeatureInfoUrl(
          ...
          viewResolution,
          projection,
          Object.assign({'QUERY_LAYERS': olLayer.get('layerId')}, this._params)
        );
    ...
    else if (this._$featuresEl.find('.aims-map-get-features-content-open').length === 0 && firstFeatureInfoElement.keepState === false) {
