...
export default class GetFeatureInfoComponent {
  ...
  constructor(olMap, layerModel) {
    ...
    this._$featuresEl = $('<div id="targetId" class="aims-map-get-features"><p>Click on the map to request a feature</p></div>');
  }
  ...
  request(event) {
    ...
    this._clickMarker.set(event.coordinate);
    ...
    this._olMap.forEachLayerAtPixel(pixel, olLayer => {
      ...
      if (layer) {
        ...
        if (url) {
          ...
          this._$featuresEl.append($containerEl);
    ...
    this._$featuresEl.find('.aims-map-get-features-container').first().find('.aims-map-get-feature-info-content').show();
  }
}
