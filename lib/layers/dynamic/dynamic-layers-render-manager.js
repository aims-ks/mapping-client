...
export default class DynamicLayersRenderManager {
   ...
   * identified by the `id` property) is represented multiple times, only the first
  ...
  _renderLayer() {
    ...
    let renderedIds = {};
    let data = this._dataModel.getData();
        ...
        if (_style) {
          const feature = new olFeature({
            geometry: new olPointGeom([
              _dataItem.longitude || _dataItem.lon,
              _dataItem.latitude || _dataItem.lat
            ])
          });
