...
export default class DynamicLayerRenderer {
  ...
  renderLayer(siteHashMap) {
    ...
    const features = [];
    Object.keys(siteHashMap).forEach(function (_siteId) {
      const _site = siteHashMap[_siteId];

      // Identify the style to use for the feature.
      const _style = this._featureStyler.identifyStyle(_site);
      if (_style) {

        // Instantiate the site as a feature.
        const feature = new olFeature({
          geometry: new olPointGeom([
            _site.longitude || _site.lon,
            _site.latitude || _site.lat
          ])
        });
        feature.setStyle(_style.clone());

        // Cache the underlying data object for future use (such as when selecting sites).
        feature.set('aimsDataItem', _site, true);
