...
/**
 * Abstract base implementation of the feature selector for dynamic layers. Instantiate one of the
 * concrete subclasses based on desired behaviour.
 *
 * @see AccumulatingDynamicLayersFeatureSelector
 * @see ToggleDynamicLayersFeatureSelector
 */
export class DynamicLayersFeatureSelector {
      ...
      featureSelectionsModel.on(featureSelectionsModel.EVENT_DATA_CHANGED, (model) => {
        ...
        for (const _id in modelData) {
          ...
          this._selectedFeatures[_id] = {
            id: _id,
            latitude: featureData.latitude,
            longitude: featureData.longitude
          };
        }
      });
    }
    ...
    this._vectorSource.forEachFeatureIntersectingExtent(_extent, function (_feature) {
      ...
      if (!this._selectedFeatures[aimsDataItem.id]) {
        ...
        this._selectedFeatures[aimsDataItem.id] = {
          id: aimsDataItem.id,
          latitude: aimsDataItem.latitude,
          longitude: aimsDataItem.longitude
        };
        debugger;
  ...
  _handleOnSelectEvent(event) {
    ...
    event.selected.forEach((_feature) => {
      ...
      // Only select aimsDataItems and ignore rest
      if (aimsDataItem) {
...
/**
 * Extends {@link DynamicLayersFeatureSelector} to provide functionality that toggles selections
 ...
 */
export class ToggleDynamicLayersFeatureSelector extends DynamicLayersFeatureSelector {
...
/**
 * Extends {@link DynamicLayersFeatureSelector} to provide functionality that accumulates selections
 * between mouse actions. For example, click a feature and it is selected, click another feature
 * and both features are selected.
 */
export class AccumulatingDynamicLayersFeatureSelector extends DynamicLayersFeatureSelector {
