export default class ExternalStaticLayersAPI {
  ...
  getAvailableBaseLayers() {
    return this._availableBaseLayersModel.getLayers();
  }

  removeActiveBaseLayer(layerId) {
    const extLayerId = 'extBaseLayer_' + layerId;
    this._activeBaseLayersModel.setLayers(this._activeBaseLayersModel.getLayers()
      .filter((layer) => layer.layerId !== extLayerId));
  }

  removeAvailableBaseLayer(layerId) {
    const extLayerId = 'extBaseLayer_' + layerId;
    this._availableBaseLayersModel.setLayers(this._availableBaseLayersModel.getLayers()
      .filter((layer) => layer.layerId !== extLayerId));
  }
  ...
  removeActiveLayer(layerId) {
    const extLayerId = 'extOverlayLayer_' + layerId;
