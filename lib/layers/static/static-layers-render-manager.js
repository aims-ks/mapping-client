...
export default class StaticLayersRenderManager {
  ...
  _handleModelChange() {
    ...
    this._layerModel.getLayers().forEach((_layer) => {
      ...
      const olRenderedLayer = this._olRenderedLayers[_layer.layerId];
      if (olRenderedLayer) {
        olRenderedLayer.setOpacity(_layer.opacity);
        if (_layer.time) {
          olRenderedLayer.getSource().updateParams({ TIME: _layer.time });
        }

        // Layer has been previously rendered, so use it.
        tempLayerList[_layer.layerId] = olRenderedLayer;

      } else {

        // Not already rendered, so render a new layer.
        const dataSource = this._supportedDataSources[_layer.dataSourceId];
        if (dataSource) {
          ...
          if (olLayer) {
            tempLayerList[_layer.layerId] = olLayer;
            _layer.olRenderedLayer = olLayer;
          }
