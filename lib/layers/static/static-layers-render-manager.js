...
export default class StaticLayersRenderManager {
  ...
  _handleModelChange() {
    ...
    this._layerModel.getLayers().forEach((_layer) => {
      ...
      if (_olRenderedLayer) {
        _olRenderedLayer.setOpacity(_layer.opacity);
        if (_layer.time) {
          _olRenderedLayer.getSource().updateParams({TIME: _layer.time});
        }
        ...
        if (_dataSource) {
          ...
          if (_olLayer) {
            ...
            _layer.olRenderedLayer = _olLayer;
          }
