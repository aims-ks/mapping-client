...
export default class StaticLayersAPI {
  ...
  addAvailableLayer(layerId) {
    const _layer = this._supportedLayersModel.findByLayerId(layerId);
    if (_layer) {
      this._downloadLayerInformation(_layer);
   ...
   * @private
   */
  _downloadLayerInformation(layer) {
    ...
    $.ajax({
      ...
      if (cachedLayer) {
        ...
        this._availableLayersModel.updateLayer(layer);
        if (this._activeLayersModel.findByLayerId(layer.layerId)) {
          this._activeLayersModel.updateLayer(layer);
        }
      }
    });
  }
