...
export default class SimpleLayerModel extends EventEmitter {
  ...
  findByLayerId(layerId) {
    return this._internalCache.find((_layer) => _layer.layerId === layerId);
  }
  ...
  /**
   * Getter method for the {@link #_internalCache} property.
   */
  getLayers() {
   ...
   * present more than once, in which case only the first occurrence of the layer is retained.
   */
  setLayers(layers) {
    const tempList = [];
    ...
    layers.slice()
      .forEach((_layer) => {
        ...
        const tempLayer = tempList.find((_findLayer) => _findLayer.layerId === _layer.layerId);
        if (!tempLayer) {
          // Not previously added, so add.
          tempList.push(_layer);
        }
      });

    // Update the internal cache.
    this._internalCache = tempList;
